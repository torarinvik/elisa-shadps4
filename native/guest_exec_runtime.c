...
// Patch-time table: each relocation patched with the AeroLib fallback stub is
// recorded here with its GOT slot VA, the (usually empty) ELF symbol name, the
// .dynsym symbol index, the relocation type and the owning module name. PS4
// JUMP_SLOT relocations frequently carry st_name==0, so the symbol *index* and
// module are the reliable identifiers; the index can be cross-referenced
// against the module's .dynsym/import tables offline.
#define ELISA_GUEST_EXEC_FALLBACK_SLOT_CAP 8192
...
static const char* elisa_guest_exec_fallback_slot_modules[ELISA_GUEST_EXEC_FALLBACK_SLOT_CAP];
static uint32_t elisa_guest_exec_fallback_slot_symbol_indices[ELISA_GUEST_EXEC_FALLBACK_SLOT_CAP];
static uint32_t elisa_guest_exec_fallback_slot_rel_types[ELISA_GUEST_EXEC_FALLBACK_SLOT_CAP];
static uint32_t elisa_guest_exec_fallback_slot_count = 0;

static int32_t elisa_guest_exec_find_fallback_slot_index(uint64_t slot_va) {
    if (slot_va == 0) {
        return -1;
    }
...
void ElisaGuestExec_RecordFallbackSlot(uint64_t slot_va, const char* name,
                                       uint32_t symbol_index, uint32_t rel_type,
    ...
    int32_t existing = elisa_guest_exec_find_fallback_slot_index(slot_va);
    if (existing >= 0) {
        uint32_t idx = (uint32_t)existing;
        if ((elisa_guest_exec_fallback_slot_names[idx] == NULL ||
             elisa_guest_exec_fallback_slot_names[idx][0] == '\0') &&
            name != NULL && name[0] != '\0') {
            elisa_guest_exec_fallback_slot_names[idx] = name;
        }
        if (elisa_guest_exec_fallback_slot_symbol_indices[idx] == 0 && symbol_index != 0) {
            elisa_guest_exec_fallback_slot_symbol_indices[idx] = symbol_index;
        }
        if (elisa_guest_exec_fallback_slot_rel_types[idx] == 0 && rel_type != 0) {
            elisa_guest_exec_fallback_slot_rel_types[idx] = rel_type;
        }
        if ((elisa_guest_exec_fallback_slot_modules[idx] == NULL ||
             elisa_guest_exec_fallback_slot_modules[idx][0] == '\0') &&
            module_name != NULL && module_name[0] != '\0') {
            elisa_guest_exec_fallback_slot_modules[idx] = module_name;
        }
        return;
    }
    ...
    uint32_t idx = elisa_guest_exec_fallback_slot_count;
    elisa_guest_exec_fallback_slot_addrs[idx] = slot_va;
    elisa_guest_exec_fallback_slot_names[idx] = name != NULL ? name : "";
    elisa_guest_exec_fallback_slot_modules[idx] = module_name != NULL ? module_name : "";
    elisa_guest_exec_fallback_slot_symbol_indices[idx] = symbol_index;
    elisa_guest_exec_fallback_slot_rel_types[idx] = rel_type;
...
uint32_t ElisaGuestExec_FallbackSlotCount(void) {
    return elisa_guest_exec_fallback_slot_count;
}

uint64_t ElisaGuestExec_FallbackSlotVaAt(uint32_t index) {
    return index < elisa_guest_exec_fallback_slot_count
        ? elisa_guest_exec_fallback_slot_addrs[index] : 0;
}

const char* ElisaGuestExec_FallbackSlotSymbolAt(uint32_t index) {
    return index < elisa_guest_exec_fallback_slot_count
        ? elisa_guest_exec_fallback_slot_names[index] : NULL;
}

const char* ElisaGuestExec_FallbackSlotModuleAt(uint32_t index) {
    return index < elisa_guest_exec_fallback_slot_count
        ? elisa_guest_exec_fallback_slot_modules[index] : NULL;
}

uint32_t ElisaGuestExec_FallbackSlotSymbolIndexAt(uint32_t index) {
    return index < elisa_guest_exec_fallback_slot_count
        ? elisa_guest_exec_fallback_slot_symbol_indices[index] : 0;
}

uint32_t ElisaGuestExec_FallbackSlotRelTypeAt(uint32_t index) {
    return index < elisa_guest_exec_fallback_slot_count
        ? elisa_guest_exec_fallback_slot_rel_types[index] : 0;
}

const char* ElisaGuestExec_FindFallbackSymbolBySlot(uint64_t slot_va) {
    int32_t index = elisa_guest_exec_find_fallback_slot_index(slot_va);
    return index >= 0 ? elisa_guest_exec_fallback_slot_names[(uint32_t)index] : NULL;
}

const char* ElisaGuestExec_FindFallbackModuleBySlot(uint64_t slot_va) {
    int32_t index = elisa_guest_exec_find_fallback_slot_index(slot_va);
    return index >= 0 ? elisa_guest_exec_fallback_slot_modules[(uint32_t)index] : NULL;
}

uint32_t ElisaGuestExec_FindFallbackSymbolIndexBySlot(uint64_t slot_va) {
    int32_t index = elisa_guest_exec_find_fallback_slot_index(slot_va);
    return index >= 0 ? elisa_guest_exec_fallback_slot_symbol_indices[(uint32_t)index] : 0;
}

uint32_t ElisaGuestExec_FindFallbackRelTypeBySlot(uint64_t slot_va) {
    int32_t index = elisa_guest_exec_find_fallback_slot_index(slot_va);
    return index >= 0 ? elisa_guest_exec_fallback_slot_rel_types[(uint32_t)index] : 0;
}

int32_t ElisaGuestExec_FindFallbackSlotIndexBySlot(uint64_t slot_va) {
    return elisa_guest_exec_find_fallback_slot_index(slot_va);
}
